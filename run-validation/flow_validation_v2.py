...
def main(flow_file, charge_only):
    ...
    with PdfPages(output_pdf_name) as output:
        ...
        if not charge_only:
           ...
           # Plot density of channel IDs versus maximum values
           plt.figure(figsize=(10, 6))
           plt.hist2d(channel_ids, max_values, bins=[64, 70], range=[[0, 64], [np.min(max_values), np.max(max_values)]], cmap='viridis', cmin=1)
           plt.colorbar(label='Counts')
           plt.xlabel('Channel ID')
           plt.ylabel('Maximum Value')
           plt.title('Density of Maximum Values vs. Channel IDs')
           # plt.grid(True) # Grid can be distracting on 2D hist, but user likes grids. Leaving it off for cleaner density map usually, or overlay. I will omit grid for density map clarity unless requested.
           output.savefig()
           plt.close()
        ...
        fig.subplots_adjust(left=0.15,bottom=0.1,wspace=None, hspace=0.)
        ...
        # New 2D plot: Packet Type vs Timestamp
        plt.figure(figsize=(10, 8))
        plt.hist2d(packets['timestamp'], packets['packet_type'], bins=[100, 8], range=[[min(packets['timestamp']), max(packets['timestamp'])], [0, 8]], cmap='viridis', cmin=1)
        plt.colorbar(label='Number of Packets')
        plt.title('Packet Type vs Timestamp')
        plt.xlabel('Timestamp')
        ...
        if 'mc_truth' in flow_h5.keys():
            ...
            # Pie chart for packet fractions
            epsilon = 1e-5
            less_than_one = np.sum(summed_fractions < (1.0 - epsilon))
            greater_than_one = np.sum(summed_fractions > (1.0 + epsilon))
            exactly_one = len(summed_fractions) - less_than_one - greater_than_one
            
            labels = [f'< 1 ({less_than_one})', f'> 1 ({greater_than_one})', f'= 1 ({exactly_one})']
            sizes = [less_than_one, greater_than_one, exactly_one]
            ...
            ax.set_title("Proportion of hits with fraction sum < 1, > 1, and = 1")
            output.savefig()
            plt.close()
