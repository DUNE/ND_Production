...
def run_gen(sh, args):
    ...
    if args.use_dk2nu:
        ...
        maxmb = 300
    print(
        "${ND_PRODUCTION_DIR}/bin/copy_dune_flux --top %s --flavor %s --maxmb=100 %s"
        % (args.fluxdir, mode, fluxopt),
        file=sh,
    )
    ...
    print(
        'sed "s/<beampos> ( 0.0, 0.05387, 6.66 )/<beampos> ( %1.2f, 0.05387, 6.66 )/g" ${ND_PRODUCTION_CONFIG}/GNuMIFlux.xml > GNuMIFlux.xml'
        % args.oa,
        file=sh,
    )
    if args.large_flux_window:
        print(
            """sed -i '/<point coord="det"> /s/-6.0,\s*-5.0,\s*-1.0/-45.0, -18.0, -40.0/g' GNuMIFlux.xml""",
            file=sh,
        )
        print(
            """sed -i '/<point coord="det"> /s/6.0,\s*-5.0,\s*-1.0/15.0, -18.0, -40.0/g' GNuMIFlux.xml""",
            file=sh,
        )
        print(
            """sed -i '/<point coord="det"> /s/6.0,\s*5.0,\s*-1.0/15.0, 27.0, -40.0/g' GNuMIFlux.xml""",
            file=sh,
        )
    ...
    if args.b_field_location != None:
        assert (
            args.b_field_filename != None
        ), "Expected a b_field_filename in conjunction with b_field_location"
    ...
    if "v3" in args.genie_tune:
        if args.manual_genie_xsec_file != None:
            ...
            if args.use_big_genie_file:
                print(
                    "GENIEXSECFILETOUSE=`dirname $GENIEXSECFILE`/gxspl-FNALbig.xml.gz",
                    file=sh,
                )
            else:
    ...
    if args.manual_geometry_override != None:
        ...
        if args.anti_fiducial:
            ...
            print(
                "    -g ${ND_PRODUCTION_GDML}/anti_fiducial_%s.gdml \\" % args.geometry,
                file=sh,
            )
        else:
    ...
    if "v3" in args.genie_tune:
        print(
            "    --cross-sections $GENIEXSECFILETOUSE --tune $GENIE_XSEC_TUNE ", file=sh
        )
    else:
...
def run_tms(sh, args):
    ...
    print(
        "$INPUT_TAR_DIR_LOCAL/dune-tms/bin/ConvertToTMSTree.exe ${EDEP_OUTPUT_FILE}",
        file=sh,
    )
    # Copy the output
    print(
...
#exit $exit_code
    ...
    print(
        "TMS_OUTPUT_FILE=`ls ${EDEP_OUTPUT_FILE/.root/_TMS_RecoCandidates_*.root}`",
        file=sh,
    )
    ...
    print(
        "TMS_PDF_FILE=`ls ${EDEP_OUTPUT_FILE/.root/_*.pdf}`", file=sh
    )  # Now try to get the pdf file
...
def run_g4(sh, args):
    ...
    if args.overlay:
        print(
            'MEAN=$(echo "std::cout << gtree->GetEntries()*(%3.3g/%3.3g) << std::endl;" | genie -l -b input_file.ghep.root 2>/dev/null  | tail -1)'
            % (args.spill_pot, args.pot),
            file=sh,
        )
        print(
            'NSPILL=$(echo "std::cout << (int)floor(0.9*gtree->GetEntries()/${MEAN}) << std::endl;" | genie -l -b input_file.ghep.root 2>/dev/null  | tail -1)',
            file=sh,
        )

        # change the macro to use mean
        print(
            'sed "s/count\/set fixed/count\/set mean/g" ${ND_PRODUCTION_CONFIG}/dune-nd.mac > dune-nd.mac',
            file=sh,
        )
        print(
            'sed -i "s/count\/fixed\/number 1/count\/mean\/number ${MEAN}/g" dune-nd.mac',
            file=sh,
        )
    else:
        print(
        ...
        if args.event_multiplicity > 1:
            print(
                'sed -i "s/count\/fixed\/number 1/count\/fixed\/number %s/g" dune-nd.mac'
                % args.event_multiplicity,
                file=sh,
            )

    should_simulate_spill_time = False
    if args.timing == "spill":
        should_simulate_spill_time = True
    elif args.timing == "fixed":
        should_simulate_spill_time = False
    elif args.timing == "default":
        # By default we only want a random time more than one event per spill
        if args.overlay:
            should_simulate_spill_time = True
        elif args.event_multiplicity > 1:
            should_simulate_spill_time = True
        else:
            should_simulate_spill_time = False
    else:
        raise ValueError("Don't understand timing option '%s'" % args.timing)
    if should_simulate_spill_time:
        ...
        print(
            "echo /generator/time/spill/start %s ns >> dune-nd.mac" % args.spill_start,
            file=sh,
        )
        print(
            "echo /generator/time/spill/bunchSep %s ns >> dune-nd.mac"
            % args.bunch_separation,
            file=sh,
        )
        print(
            "echo /generator/time/spill/bunchLength %s ns >> dune-nd.mac"
            % args.bunch_length,
            file=sh,
        )
        print(
            "echo /generator/time/spill/bunchCount %s >> dune-nd.mac"
            % args.bunch_count,
            file=sh,
        )
        print("echo /generator/time/set spill >> dune-nd.mac", file=sh)
        print(
            "echo /generator/add >> dune-nd.mac", file=sh
        )  # Makes sure to update the generator based on parameters
...
def run_larcv(sh, args):
    ...
    if not any(x in stages for x in ["g4", "geant4", "edepsim", "edep-sim"]):
        ...
        print(
            "ifdh cp %s/edep/%s/%02.0fm/${RDIR}/%s.${RUN}.edep.root %s.${RUN}.edep.root"
            % (args.indir, args.horn, args.oa, mode, mode),
            file=sh,
        )
    ...
    print(
        "python ${supera_dir}/run_supera.py reco/larcv.cfg %s.${RUN}.edep.root" % mode,
        file=sh,
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--horn_current",
        help="horn current (default is 300 for FHC, -300 for RHC",
        type="float",
        default=None,
    )
    ...
    parser.add_option(
        "--topvol",
        help="Top volume for generating events (gen stage only)",
        default="volWorld",
    )
    parser.add_option("--pot", help="POT per job", type="float", default=1.0e16)
    parser.add_option("--spill_pot", help="POT per spill", type="float", default=7.5e13)
    parser.add_option(
        "--first_run", help="First run number to use", default=0, type="int"
    )
    parser.add_option(
        "--oa", help="Off-axis position in meters", default=0, type="float"
    )
    parser.add_option(
        "--large_flux_window",
        help="Use a larger flux window",
        default=False,
        action="store_true",
    )
    parser.add_option(
        "--test",
        help="Use test mode (interactive job)",
        default=False,
        action="store_true",
    )
    parser.add_option(
        "--overlay",
        help="Simulate full spills (default is single events)",
        default=False,
        action="store_true",
    )
    parser.add_option(
        "--timing",
        help="Simulate a random time in spill. default/fixed/spill (default is 1ns for single events and a random spill time for overlay or event_multiplicity > 1). ",
        default="default",
    )
    parser.add_option(
        "--event_multiplicity",
        help="The fixed number of events. Overwritten with overlay option. Useful if you want say a fixed 5 events per spill",
        default=1,
        type="int",
    )
    parser.add_option(
        "--stages",
        help="Production stages (gen+g4+larcv+ana+tmsreco)",
        default="gen+g4+larcv+ana+tmsreco",
    )
    parser.add_option(
        "--persist",
        help="Production stages to save to disk(gen+g4+larcv+ana+tmsreco)",
        default="all",
    )
    parser.add_option(
        "--indir",
        help="Input file top-directory (if not running gen)",
        default="/pnfs/dune/persistent/users/%s/nd_production" % user,
    )
    parser.add_option(
        "--fluxdir",
        help="Specify the top-level flux file directory",
        default="/cvmfs/dune.osgstorage.org/pnfs/fnal.gov/usr/dune/persistent/stash/Flux/g4lbne/v3r5p4/QGSP_BERT/OptimizedEngineeredNov2017",
    )
    parser.add_option(
        "--outdir",
        help="Top-level output directory",
        default="/pnfs/dune/scratch/users/%s/nd_production" % user,
    )
    parser.add_option(
        "--use_dk2nu",
        help="Use full dk2nu flux input (default is gsimple)",
        action="store_true",
        default=False,
    )
    parser.add_option(
        "--sam_name",
        help="Make a sam dataset with this name",
        default="dune_nd_miniproduction_2021_v1",
    )
    parser.add_option(
        "--dropbox_dir",
    ...
    parser.add_option(
        "--application_family", help="application family", default="neardet"
    )
    parser.add_option(
        "--application_name",
        help="application name",
        default="nd_production,genie,edep-sim",
    )
    parser.add_option(
        "--application_version",
        help="application version",
        default="v01_04_00,v2_12_10,v3_0_1b",
    )
    parser.add_option(
        "--campaign", help="DUNE.campaign", default="dune_nd_miniproduction_2021_v1"
    )
    parser.add_option("--requestid", help="DUNE.requestid", default="RITM1254894")
    parser.add_option(
        "--tms_reco_tar",
        help="The tar file which contains the tms reco code",
        default="/pnfs/dune/persistent/stash/ND_simulation/production_v01/dune-tms.tar.gz",
    )
    parser.add_option(
        "--sam_input", help="Use a sam dataset with this name", default=None
    )

    parser.add_option(
        "--anti_fiducial",
        help="anti fiducial using : anti_fiducial_geometry.gdml",
        default=False,
        action="store_true",
    )
    parser.add_option(
        "--manual_geometry_override",
        help="Advanced feature: Point to a specific geometry file (like in the tar input file). Useful if you want to remove rotation for example",
        default="nd_hall_with_lar_tms_sand_TDR_Production_geometry_v_1.0.3.gdml",
    )
    parser.add_option(
        "--geometry_location",
        help="Advanced feature: Point to a specific geometry file and it will be copied over. Use in conjunction with manual_geometry_override to point to the local copy after being copied over.",
        default="/pnfs/dune/persistent/physicsgroups/dunendsim/geometries/TDR_Production_geometry_v_1.0.3/nd_hall_with_lar_tms_sand_TDR_Production_geometry_v_1.0.3.gdml",
    )
    parser.add_option(
        "--b_field_location",
        help="Advanced feature: Point to a specific b field file (like in the tar input file).",
        default=None,
    )
    parser.add_option(
        "--b_field_filename",
        help="Advanced feature: Required in conjunction with b_field_location. Dictates the final name",
        default=None,
    )
    parser.add_option("--genie_tune", help="Genie version", default="v3_02_02_p01")
    parser.add_option(
        "--genie_xsec_version", help="genie_xsec version", default="v3_02_00"
    )
    parser.add_option(
        "--genie_options", help="Genie version", default="G1810a0211a:e1000:k250"
    )
    parser.add_option(
        "--genie_phyopt_version", help="Version of genie_phyopt", default="v3_02_00"
    )
    parser.add_option(
        "--genie_phyopt_options",
        help="Additional args for genie_phyopt",
        default="dkcharmtau",
    )
    parser.add_option(
        "--use_big_genie_file",
        help="whether to use gxspl-FNALbig.xml.gz",
        default=False,
        action="store_true",
    )
    parser.add_option(
        "--manual_genie_xsec_file",
        help="use this specific genie xsec file",
        default=None,
    )
    parser.add_option(
        "--spill_start", help="Spill start time in ns", type="float", default=0
    )
    parser.add_option(
        "--bunch_separation",
        help="Separation of each bunch in a spill in ns",
        type="float",
        default=19.23077,
    )
    parser.add_option(
        "--bunch_length",
        help="Length of each bunch in a spill in ns",
        type="float",
        default=0,
    )
    parser.add_option(
        "--bunch_count", help="Number of bunches in a spill", type="int", default=520
    )
    ...
    if "cvmfs" not in args.fluxdir:
        ...
        print(args.fluxdir)
        print(
            "is not in cvmfs. The flux setup probably will not work unless you really know what you are doing"
        )
    if "persistent" in args.outdir:
        print(
            "FATAL: Cannot use persistent in outdir. Please use scratch and then copy over."
        )
        exit()
    ...
    if "v3" in args.genie_tune:
        ...
        print(
            "setup genie_xsec    %s   -q %s"
            % (args.genie_xsec_version, args.genie_options),
            file=sh,
        )
        print(
            "setup genie_phyopt %s -q %s"
            % (args.genie_phyopt_version, args.genie_phyopt_options),
            file=sh,
        )
    else:
        print("setup dk2nugenie   v01_06_01f -q e17:prof", file=sh)
    ...
    if any(x in stages for x in ["gen", "genie", "generator"]):
        ...
        if args.sam_name is not None:
            ...
            if args.anti_fiducial:
                copylines.append(
                    'generate_sam_json ${GHEP_FILE} ${RUN} ${NSPILL} "generated" %s %1.2f %s %s %1.1f %d %s %s %s %s %s %s %s\n'
                    % (
                        args.sam_name,
                        args.oa,
                        args.topvol,
                        "anti_fiducial_" + args.geometry,
                        hc,
                        fluxid,
                        args.data_stream,
                        args.file_format,
                        args.application_family,
            ...
            copylines.append(
                "ifdh cp ${GHEP_FILE}.json %s/${GHEP_FILE}.json\n" % args.dropbox_dir
            )
        if args.persist == "all" or any(
            x in args.persist for x in ["gen", "genie", "generator"]
        ):
            copylines.append(
                "ifdh_mkdir_p %s/genie/%s/%02.0fm/${RDIR}\n"
                % (args.outdir, args.horn, args.oa)
            )
            copylines.append(
                "ifdh cp ${GHEP_FILE} %s/genie/%s/%02.0fm/${RDIR}/${GHEP_FILE}\n"
                % (args.outdir, args.horn, args.oa)
            )
    ...
    if any(x in stages for x in ["g4", "geant4", "edepsim", "edep-sim"]):
        ...
        if args.sam_name is not None:
            if args.anti_fiducial:
                copylines.append(
                    'generate_sam_json ${EDEP_FILE} ${RUN} ${NSPILL} "simulated" %s %1.2f %s %s %1.1f %d %s %s %s %s %s %s %s\n'
                    % (
                        args.sam_name,
                        args.oa,
                        args.topvol,
            ...
            copylines.append(
                "ifdh cp ${EDEP_FILE}.json %s/${EDEP_FILE}.json\n" % args.dropbox_dir
            )
        if args.persist == "all" or any(
            x in args.persist for x in ["g4", "geant4", "edepsim", "edep-sim"]
        ):
            copylines.append(
                "ifdh_mkdir_p %s/edep/%s/%02.0fm/${RDIR}\n"
                % (args.outdir, args.horn, args.oa)
            )
            copylines.append(
                "ifdh cp ${EDEP_FILE} %s/edep/%s/%02.0fm/${RDIR}/${EDEP_FILE}\n"
                % (args.outdir, args.horn, args.oa)
            )
    # else:
    ...
    if any(x in stages for x in ["larcv"]):
        ...
        if args.persist == "all" or any(x in args.persist for x in ["larcv"]):
            copylines.append(
                "ifdh_mkdir_p %s/larcv/%s/%02.0fm/${RDIR}\n"
                % (args.outdir, args.horn, args.oa)
            )
            copylines.append(
                "ifdh cp larcv.root %s/larcv/%s/%02.0fm/${RDIR}/%s.${RUN}.larcv.root\n"
    ...
    if any(x in stages for x in ["tmsreco"]):
        ...
        copylines.append(
            "TMS_PDF_FINAL_FILE=`basename ${EDEP_FILE/edep.root/tmsreco.pdf}`\n"
        )
        ...
        if args.sam_name is not None:
            copylines.append(
                'generate_sam_json ${TMS_FILE} ${RUN} ${NSPILL} "simulated" %s %1.2f %s %s %1.1f %d %s %s %s %s %s %s %s\n'
                % (
                    args.sam_name,
                    args.oa,
                    args.geometry,
            ...
            copylines.append(
                "ifdh cp ${TMS_FILE}.json %s/${TMS_FILE}.json\n" % args.dropbox_dir
            )

        if args.persist == "all" or any(x in args.persist for x in ["tmsreco"]):
            copylines.append(
                "ifdh_mkdir_p %s/tmsreco/%s/%02.0fm/${RDIR}\n"
                % (args.outdir, args.horn, args.oa)
            )
            copylines.append(
                "ifdh cp ${TMS_FILE} %s/tmsreco/%s/%02.0fm/${RDIR}/${TMS_FILE}\n"
                % (args.outdir, args.horn, args.oa)
            )
            copylines.append(
                "ifdh cp ${TMS_READOUT_FILE} %s/tmsreadout/%s/%02.0fm/${RDIR}/${TMS_READOUT_FILE}\n"
                % (args.outdir, args.horn, args.oa)
            )
            copylines.append(
                'if test -f "$TMS_PDF_FINAL_FILE"; then ifdh cp ${TMS_PDF_FINAL_FILE} %s/tmsreco/%s/%02.0fm/${RDIR}/${TMS_PDF_FINAL_FILE}; fi\n'
                % (args.outdir, args.horn, args.oa)
            )
    ...
    if not args.test:
        if "tmsreco" in args.stages or "all" in args.stages:
            ...
            print(
                "jobsub_submit --group dune --role=Analysis -N %s --OS=SL7 --expected-lifetime=24h --memory=4000MB --use-cvmfs-dropbox --tar_file_name=dropbox://%s file://processnd.sh"
                % (N_TO_SHOW, TMS_TAR_FILE)
            )
        else:
            print("Script processnd.sh created. Submit example:\n")
            print(
                "jobsub_submit --group dune --role=Analysis -N %s --OS=SL7 --expected-lifetime=24h --memory=4000MB file://processnd.sh"
                % N_TO_SHOW
            )
