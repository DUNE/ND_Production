...
export ND_PRODUCTION_LOGDIR_BASE="/pscratch/sd/a/abooth/MicroProdN4p1/logs"
export ND_PRODUCTION_OUTDIR_BASE="/pscratch/sd/a/abooth/MicroProdN4p1/output"

export ND_PRODUCTION_COMPRESS="gzip"
export ND_PRODUCTION_IN_NAME="MicroProdN4p1_NDComplex_FHC.larnd.full.light"
export ND_PRODUCTION_OUT_NAME="MicroProdN4p1_NDComplex_FHC.flow.full.light.v251016"
...
workflow5='yamls/ndlar_flow/workflows/charge/merged_calibration_mc.yaml'
workflow6='yamls/ndlar_flow/workflows/charge/final_calibration_mc.yaml'

# light workflows
workflow7='yamls/ndlar_flow/workflows/light/light_event_building_mc.yaml'
workflow8='yamls/ndlar_flow/workflows/light/light_event_reconstruction_mc.yaml'

# charge-light trigger matching
workflow9='yamls/ndlar_flow/workflows/charge/charge_light_assoc_mc.yaml'
...
run h5flow -c $workflow1 $workflow2 $workflow3 $workflow4 $workflow5 $workflow6\
    -i "$inFile" -o "$outFile" $compression

run h5flow -c $workflow7 $workflow8\
    -i "$inFile" -o "$outFile" $compression

run h5flow -c $workflow9\
